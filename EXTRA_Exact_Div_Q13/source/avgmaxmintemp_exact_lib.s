@;----------------------------------------------------------------
@;	"avgmaxmintemp_exact_lib.s": adaptació de les rutines de 
@;  càlcul de valors mitjans, màxims i mínims d'una taula de 
@;  temperatures, expressades en graus Celsius o Fahrenheit, en 
@;  format Q13 (coma fixa 1:18:13) amb les rutines de xlibQ13.a
@;----------------------------------------------------------------
...
avgmaxmin_city:
		push {r1 - r12, lr}									@; Es guarden r1 i r2 perquè es fan modificacions sobre aquests registres per accedir a la fila id_city i guardar
															@; la direcció de memòria necessària per poder cridar rutines de xlibQ13.a i emmagatzemar l'estat de l'overflow.
		...
		sub sp, #4											@; Es reserva l'espai en memòria per la pila, per tal de poder cridar a add_Q13 dins del bucle i poder deixar l'estat
															@; de l'overflow en alguna posició de memòria, i també per la posterior crida a exact_div_Q13.
		mov r2, sp											@; Es guarda en r2 la dir de mem per guardar l'overflow de les diferents rutines cridades de xlibQ13.a
...
@; endfor
		mov r1, #Q13_12										@; S'ha de carregar en r1 per la crida a exact_div_Q13											
		...
		bl exact_div_Q13									@; Llamada a rutina div_mod().
